...
JSAPI.prototype.clone = function () {
  ...
  // in the constructor.
  const clonedNode = new JSAPI(nodeData, Boolean(this.parentNode));

  if (this.class) {
    clonedNode.class = this.class.clone(clonedNode);
  }

  if (this.style) {
    clonedNode.style = this.style.clone(clonedNode);
  }

  if (this.children) {
    clonedNode.children = this.children.map((childNode) => {
...
JSAPI.prototype.hasAttrLocal = function (localName, val) {
  ...
  if (!arguments.length) return Boolean(this.attrs);
...
JSAPI.prototype.removeAttr = function (name, val, recursive) {
  ...
  if (Array.isArray(name)) {
    name.forEach((n) => this.removeAttr(n), this);
    return false;
  }
...
JSAPI.prototype.querySelectorAll = function (selectors) {
  const matchedEls = selectAll(selectors, this, cssSelectOpts);
