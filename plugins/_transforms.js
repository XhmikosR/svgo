...
const mth = (exports.mth = {
  ...
  acos(val, floatPrecision) {
    return Number(this.deg(Math.acos(val)).toFixed(floatPrecision));
  },
  ...
  asin(val, floatPrecision) {
    return Number(this.deg(Math.asin(val)).toFixed(floatPrecision));
  },
  ...
  atan(val, floatPrecision) {
    return Number(this.deg(Math.atan(val)).toFixed(floatPrecision));
  },
});
...
exports.matrixToTransform = function (transform, params) {
  ...
  if (!data[1] && data[2]) {
    ...
    if (rotate[0] && (data[4] || data[5])) {
      ...
      const denom = ((1 - cos) ** 2 + sin ** 2) * (scaleBefore || sx * sy);
      rotate.push(
        ((1 - cos) * x - sin * y) / denom,
        ((1 - cos) * y + sin * x) / denom
      );
