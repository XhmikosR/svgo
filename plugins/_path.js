...
exports.computeCubicBoundingBox = function (xa, ya, xb, yb, xc, yc, xd, yd) {
  ...
  let ts = computeCubicFirstDerivativeRoots(xa, xb, xc, xd);
...
exports.computeQuadraticBoundingBox = function (xa, ya, xb, yb, xc, yc) {
  ...
  let t = computeQuadraticFirstDerivativeRoot(xa, xb, xc);

  if (t >= 0 && t <= 1) {
    const x = computeQuadraticBaseValue(t, xa, xb, xc);
  ...
  if (t >= 0 && t <= 1) {
    ...
    const y = computeQuadraticBaseValue(t, ya, yb, yc);
...
exports.intersects = function (path1, path2) {
  ...
  const hullNest1 = points1.map((p) => convexHull(p));
  const hullNest2 = points2.map((p) => convexHull(p));
...
function a2c(
  ...
  const rad = (Math.PI / 180) * (Number(angle) || 0);
  let res = [];
