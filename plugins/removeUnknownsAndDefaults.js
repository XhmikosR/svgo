'use strict';

const { parseName } = require('../lib/svgo/tools.js');

exports.type = 'perItem';

exports.active = true;

exports.description =
  'removes unknown elements content and attributes, removes attrs with default values';

exports.params = {
  unknownContent: true,
  unknownAttrs: true,
  defaultAttrs: true,
  uselessOverrides: true,
  keepDataAttrs: true,
  keepAriaAttrs: true,
  keepRoleAttr: false,
};

const {
  elems,
  attrsGroups,
  elemsGroups,
  attrsGroupsDefaults,
  inheritableAttrs: attrsInheritable,
  presentationNonInheritableGroupAttrs: applyGroups,
} = require('./_collections');

// collect and extend all references
for (const elem of Object.values(elems)) {
  if (elem.attrsGroups) {
    elem.attrs = elem.attrs || [];

    elem.attrsGroups.forEach((attrsGroupName) => {
      elem.attrs = elem.attrs.concat(attrsGroups[attrsGroupName]);

      const groupDefaults = attrsGroupsDefaults[attrsGroupName];

      if (groupDefaults) {
        elem.defaults = elem.defaults || {};

        for (const [attrName, attr] of Object.entries(groupDefaults)) {
          elem.defaults[attrName] = attr;
        }
      }
    });
  }

  if (elem.contentGroups) {
    elem.content = elem.content || [];

    elem.contentGroups.forEach((contentGroupName) => {
      elem.content = [...elem.content, ...elemsGroups[contentGroupName]];
    });
  }
}

/**
 * Remove unknown elements content and attributes,
 * remove attributes with default values.
 *
 * @param {Object} item current iteration item
 * @param {Object} params plugin params
 * @return {Boolean} if false, item will be filtered out
 *
 * @author Kir Belevich
 */
exports.fn = function (item, params) {
  // elems w/o namespace prefix
  if (item.type === 'element' && !parseName(item.name).prefix) {
    const elem = item.name;

    // remove unknown element's content
    if (
      params.unknownContent &&
      elems[elem] && // make sure we know of this element before checking its children
      elem !== 'foreignObject' // Don't check foreignObject
    ) {
      item.children.forEach((content, i) => {
        if (
          content.type === 'element' &&
          !parseName(content.name).prefix &&
          ((elems[elem].content && // Do we have a record of its permitted content?
            !elems[elem].content.includes(content.name)) ||
            (!elems[elem].content && // we dont know about its permitted content
              !elems[content.name])) // check that we know about the element at all
        ) {
          item.children.splice(i, 1);
        }
      });
    }

    // remove element's unknown attrs and attrs with default values
    if (elems[elem] && elems[elem].attrs) {
      item.eachAttr((attr) => {
        const { prefix } = parseName(attr.name);
        if (
          attr.name !== 'xmlns' &&
          (prefix === 'xml' || !prefix) &&
          (!params.keepDataAttrs || !attr.name.startsWith('data-')) &&
          (!params.keepAriaAttrs || !attr.name.startsWith('aria-')) &&
          (!params.keepRoleAttr || attr.name !== 'role') &&
          // unknown attrs
          ((params.unknownAttrs && !elems[elem].attrs.includes(attr.name)) ||
            // attrs with default values
            (params.defaultAttrs &&
              !item.hasAttr('id') &&
              elems[elem].defaults &&
              elems[elem].defaults[attr.name] === attr.value &&
              (!attrsInheritable.includes(attr.name) ||
                !item.parentNode.computedAttr(attr.name))) ||
            // useless overrides
            (params.uselessOverrides &&
              !item.hasAttr('id') &&
              !applyGroups.includes(attr.name) &&
              attrsInheritable.includes(attr.name) &&
              item.parentNode.computedAttr(attr.name, attr.value)))
        ) {
          item.removeAttr(attr.name);
        }
      });
    }
  }
};
