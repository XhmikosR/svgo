...
exports.fn = function (item, params) {
  if (item.isElem(pathElems) && item.hasAttr('d')) {
    ...
    error =
      precision !== false
        ? Number((0.1 ** precision).toFixed(precision))
        : 1e-2;
  ...
  path = path.filter((item, index, path) => {
    ...
    if (data) {
      ...
      if (
        ...
        for (
          j = index;
          (next = path[++j]) && 'cs'.includes(next.instruction);
      ...
      if (params.removeUseless && !maybeHasStrokeAndLinecap) {
        ...
        if ('lhvqtcs'.includes(instruction) && data.every((i) => i === 0)) {
          path[index] = prev;
          return false;
        }
...
function strongRound(data) {
  for (let i = data.length; i-- > 0; ) {
    if (data[i].toFixed(precision) !== data[i]) {
      ...
      data[i] =
        Number(Math.abs(rounded - data[i]).toFixed(precision + 1)) >= error
